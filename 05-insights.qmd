...
| 08 Embeddings | Family | Could go to SKU but at 66 SKUs the embedding space gets sparse. |
...
2. **Co-purchase patterns are strong and exploitable** — once filtered to the *actionable* set in chapter 01. Top examples after the triage: `dining_table → dining_chair` (81%, lift 4.9, asymmetric), `kitchen_table → kitchen_chair` (74%, lift 9.5, asymmetric), `garden_table → garden_chair` (67%, lift 14.2, asymmetric). Filtered out as plumbing: `bed → mattress` (symmetric — you can't really have one without the other), `headboard → bed` (accessory implies primary). Both fire at high raw confidence but encode catalog structure, not customer choice. Even the *misses* on the actionable rules are quantifiable: 52 of 271 `dining_table` baskets did not include chairs — at an average chair price of €153 that's roughly **€8,000 of forgone revenue** from a single rule, on a single product.
