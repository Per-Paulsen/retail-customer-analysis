...
# Apply triage — same logic as chapter 01
bundle_lookup = (
    df.drop_duplicates("article_name").set_index("article_name")["bundle_group"]
      .fillna("").to_dict()
)
conf_map_simple = {(r["antecedent"], r["consequent"]): r["confidence"] for _, r in simple.iterrows()}
simple["a_bundle"] = simple["antecedent"].map(bundle_lookup)
simple["c_bundle"] = simple["consequent"].map(bundle_lookup)
simple["within_bundle"] = (
    (simple["a_bundle"] != "") & (simple["a_bundle"] == simple["c_bundle"])
)
simple["reverse_conf"] = simple.apply(
    lambda r: conf_map_simple.get((r["consequent"], r["antecedent"]), 0.0), axis=1
)
...
simple = simple[~simple["within_bundle"] & (simple["symmetry"] < 0.75)].copy()
...
2. **Co-purchase patterns are strong and exploitable** — once filtered to *actionable* rules (cross-bundle, asymmetric). Top examples after the triage in chapter 01: `dining_table → dining_chair` (81%, lift 4.9), `kitchen_table → kitchen_chair` (74%, lift 9.5), `garden_table → garden_chair` (67%, lift 14.2). Within-bundle pairs like `bed → mattress` and `headboard → bed` *also fire at high confidence* but are filed as definitional plumbing — you can't have a bed without a mattress, so the rule discovers the catalog structure rather than customer behavior. Even the *misses* on the actionable rules are quantifiable: 52 of 271 `dining_table` baskets did not include chairs — at an average chair price of €153 that's roughly **€8,000 of forgone revenue** from a single rule, on a single product.
