...
basket = first_tx_lines.groupby("customer_id").agg(
    ...
    first_top_group=("product_group", lambda s: s.mode().iloc[0]),
)
...
```{python}
#| label: t-learner
data_enc = pd.get_dummies(data, columns=["first_top_group"], drop_first=True, dtype=int)
feature_cols = (
    ["first_basket_value", "first_n_items"]
    + [c for c in data_enc.columns if c.startswith("first_top_group_")]
)
