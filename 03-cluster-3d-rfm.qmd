...
Reference date: 2017-06-30 (the last day of the data window). Recency is measured in months since the article's last sale.

```{r}
#| label: rfm-features
raw <- read_delim("data/synthetic/transactions.csv", delim = ";",
                  show_col_types = FALSE)

REF_DATE <- as.Date("2017-06-30")
...
cat(sprintf("%d articles, recency range [%.1f, %.1f] months, value range [%.0f, %.0f] EUR\n",
            nrow(rfm), min(rfm$recency), max(rfm$recency),
...
Most articles have very recent last sales (the data window ends 2017-06-30, and most products are still selling). A small tail of articles last sold months ago — these are the declining products we engineered into the synthesis (`filing_cabinet`, `dvd_player`).
