...
### Triage — separating insight from plumbing

Looking at the top rules above, *some are actionable insight* (`dining_table → dining_chair` is a cross-sell prompt waiting to be wired), but *some are definitional plumbing* (`bed → mattress` is the textbook example of a rule that confirms how furniture works rather than how customers behave).

To separate them mechanically, we annotate each rule with two signals from the [data audit](00-data-audit.qmd):

- **Bundle membership** — if antecedent and consequent are in the same `bundle_group`, the rule is a within-system pair. Within-bundle co-purchase is mostly definitional.
- **Symmetry score** — the smaller of the two confidences (A→B, B→A) divided by the larger. Rules near 1.0 are *symmetric* (you can't have one without the other → definitional). Asymmetric rules are the actionable ones — *one side drives the other*.

```{python}
#| label: triage-rules
# Bundle group lookup
bundle_lookup = (
    df.drop_duplicates("article_name").set_index("article_name")["bundle_group"].fillna("").to_dict()
)

# Reverse-confidence lookup: for each rule A→B, find the matching rule B→A and read its confidence
...
simple["within_bundle"] = (
    (simple["a_bundle"] != "")
    & (simple["a_bundle"] == simple["c_bundle"])
)
...
simple.head(12)[["antecedent", "consequent", "confidence", "lift",
                 "reverse_conf", "symmetry", "within_bundle", "triage"]]\
    .round({"confidence": 3, "lift": 2, "reverse_conf": 3, "symmetry": 2})
```

### Top *actionable* rules

The actionable rules — asymmetric AND cross-bundle — are the ones a stakeholder should see:
...
These are the rules where one product *drives* purchase of another in a non-obvious way. Notice that `bed → mattress` and `headboard → bed` have dropped out — they're filed as plumbing, not insight. What remains is the cross-sell map you'd actually deploy.
