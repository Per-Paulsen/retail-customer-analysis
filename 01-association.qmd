...
- **Reverse confidence + symmetry score** — `min(conf(A→B), conf(B→A)) / max(...)`. Symmetric pairs (≥ 0.7) are "you can't really have one without the other" (`bett ↔ matratze`). Asymmetric pairs are real cross-sell directionality.
...
*"Which items belong to the same product system?"* — useful for set-merchandising, "complete your set" UI, and inventory of complementary parts. We surface within-bundle *or* substring-related pairs, sorted by symmetry × lift so the strongest semantic pairings come out on top.

```{python}
#| label: view-bundle-composition
bundle_view = (
    simple[(simple["within_bundle"]) | (simple["substring_pair"])]
    .assign(score=lambda d: d["symmetry"] * d["lift"])
    .sort_values("score", ascending=False)
)
bundle_view.head(15)[["antecedent", "consequent", "support", "confidence",
                      "reverse_conf", "symmetry", "lift", "within_bundle", "substring_pair"]]\
...
*"Which items pull in another item from a different system?"* — the deployable cross-sell-trigger list. Filter: cross-bundle (or no-bundle), no substring relation, asymmetric (`symmetry < 0.7`), lift ≥ 2.
...
*"What are the strongest patterns in the data, period?"* — for stakeholder reports. Sort by lift, annotate each rule with its bundle / substring class so the reader can interpret it correctly:
