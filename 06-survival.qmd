...
Beyond the population, we want to know if first-purchase characteristics shift the curve. Cohort by the **product group of the first basket** and compare:
...
surv_full[["duration", "event", "first_basket_value", "first_n_items",
           "first_top_group", "first_had_discount"]].describe(include="all").round(2)
```

```{python}
#| label: fig-km-by-group
#| fig-cap: "Survival curves stratified by the first basket's dominant product group. Overlapping curves and confidence intervals = no real differences between groups."
top_groups = surv_full["first_top_group"].value_counts().head(5).index.tolist()

fig, ax = plt.subplots(figsize=(9, 5))
palette = sns.color_palette("tab10", n_colors=len(top_groups))
for g, c in zip(top_groups, palette):
    mask = surv_full["first_top_group"] == g
    n = int(mask.sum())
...
ax.set_ylabel("P(no second purchase yet)")
ax.set_ylim(0.55, 1.02)
ax.legend(title="First-basket dominant group", fontsize=9)
plt.tight_layout()
plt.show()
```
...
result = multivariate_logrank_test(
    surv_full["duration"], surv_full["first_top_group"], surv_full["event"],
)
print(f"Multivariate log-rank test across product groups:")
print(f"  test statistic:  {result.test_statistic:.2f}")
print(f"  p-value:         {result.p_value:.4f}")
```

If the curves were materially different, we'd see clear separation and a small p-value. Here they sit on top of one another and the log-rank test fails to reject the null — *the first basket's category does not predict the return timing in this dataset.*
...
cph_data = pd.get_dummies(cph_data, columns=["first_top_group"], drop_first=True, dtype=int)

feature_cols = (
    ["log_first_value", "first_n_items", "first_had_discount"]
    + [c for c in cph_data.columns if c.startswith("first_top_group_")]
)
cph_input = cph_data[["duration", "event"] + feature_cols]
