...
```{python}
#| label: fig-elbow
#| fig-cap: "Elbow plot — within-cluster sum of squares as a function of k. The classic BCG framework distinguishes four quadrants (Stars / Cash Cows / Question Marks / Dogs); we use k=4 for that mapping. Whether the elbow visually agrees depends on the data: synth has a clean bend at k=4, real-data is usually less sharply elbowed and the choice of k=4 is then framework-driven, not data-driven."
...
We use **k = 4** to map onto the four BCG quadrants. On synthetic data the elbow lines up cleanly with that choice; on real data the curve is often smoother and the k=4 decision becomes framework-driven (we want exactly four tiers for executive reporting) rather than purely data-driven.
...
```{python}
#| label: fig-bcg
#| fig-cap: "Products positioned by transformed share × growth. Quadrants split at the data-driven mean of each axis. Cluster numbers are ranked: 1 = top performers, 4 = lagging. On real data (~2,100 products) only the top performers are annotated to keep the plot readable; all points are still in the cluster fit."
def signed_cbrt(x):
